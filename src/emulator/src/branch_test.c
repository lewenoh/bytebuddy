...
void testbranch() {
	...
	testcond(p.pc==0x2, "Branch forward by 2");
	...
	testcond(p.pc==0x1, "Branch reg to 0x1");
	...
	testcond(p.pc==0x35, "Branch EQ forward by 3");

	branch(p, BRANCH_COND2);
	testcond(p.pc==0x35, "Branch GT condition fails");

	p.pstate[0] = true;
	testcond(p.pc = 0x31, "LT holds, backward by 4");
}
